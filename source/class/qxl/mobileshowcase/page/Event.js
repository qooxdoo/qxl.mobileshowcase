...
qx.Class.define("qxl.mobileshowcase.page.Event", {
  ...
  members: {
    ...
    _initialize() {
      ...
      var container = (this.__showcaseContainer =
        new qx.ui.mobile.container.Composite(
          new qx.ui.mobile.layout.VBox().set({
            alignX: "center",
            alignY: "middle",
          })
        ));
      ...
      var containerTouchArea = (this.__container =
        new qx.ui.mobile.container.Composite(
          new qx.ui.mobile.layout.VBox().set({
            alignX: "center",
            alignY: "middle",
      ...
      this.__gestureTarget = new qx.ui.mobile.basic.Image(
      ...
      var label = (this.__label = new qx.ui.mobile.basic.Label(
        "Touch / Tap / Swipe this area"
      ));
      containerTouchArea.add(label);

      var descriptionText =
        "<b>Testing Pointer Events:</b> Touch / Tap / Swipe the area<br />\n\
      ...
      qx.bom.element.Style.set(
        this.__container.getContentElement(),
        "background",
        "-" +
          this.__vendorPrefix +
          "-radial-gradient(50% 50%, cover, #1a82f7, #2F2727)"
      );
    ...
    _updatePointerPosition(evt) {
      ...
      this._setPointerCirclePosition(
        evt.getPointerId(),
        position[0],
        position[1]
      );
     ...
     */
    _resetPointerPosition(pointerId) {
     ...
     */
    _setPointerCirclePosition(pointerId, x, y) {
    ...
    _getPointerPosition(evt) {
      var containerLeft = qx.bom.element.Location.getLeft(
        this.__container.getContentElement(),
        "padding"
      );
      var containerTop = qx.bom.element.Location.getTop(
        this.__container.getContentElement(),
        "padding"
      );
      return [
        evt.getViewportLeft() - containerLeft,
        evt.getViewportTop() - containerTop,
      ];
    },
    ...
    _renderLogo() {
      ...
      var transitionValue =
        "translate(" + this.__logoLeft + "px," + this.__logoTop + "px) ";
      transitionValue = transitionValue + " scale(" + this.__currentScale + ")";
      transitionValue =
        transitionValue + " rotate(" + this.__currentRotation + "deg)";
    ...
    _renderLabel() {
      ...
      for (var pointerId in this.__pointers) {
        ...
        labelBuffer =
          labelBuffer + "<span class='pointer'>" + pointerId + "</span>";
