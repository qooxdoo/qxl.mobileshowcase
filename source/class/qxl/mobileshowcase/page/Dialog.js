...
qx.Class.define("qxl.mobileshowcase.page.Dialog", {
  ...
  members: {
    ...
    _initialize() {
      ...
      this.__resultsLabel = new qx.ui.mobile.basic.Label(
      ...
      closeDialogButton1.addListener(
        "tap",
        function () {
          this.__popup.hide();
        },
      ...
      this.__menu.addListener(
        "changeSelection",
        this.__onMenuChangeSelection,
        this
      );
      ...
      showAnchorMenuButton.addListener(
        "tap",
        function (e) {
      ...
      this.__anchorMenu = new qx.ui.mobile.dialog.Menu(
        anchorMenuModel,
        showAnchorMenuButton
      );
      this.__anchorMenu.setTitle("Colors");
      this.__anchorMenu.addListener(
        "changeSelection",
        this.__onMenuChangeSelection,
        this
      );
      ...
      busyIndicatorButton.addListener(
        "tap",
        function (e) {
     ...
     */
    _createPicker(anchor) {
      var pickerDialog = (this.__pickerDialog = new qx.ui.mobile.dialog.Popup(
        anchor
      ));
      ...
      picker.addListener(
        "changeSelection",
        this.__onPickerChangeSelection,
        this
      );

      this.__pickerDaySlotData = this._createDayPickerSlot(
        0,
     ...
     */
    _createDayPickerSlot(month, year) {
     ...
     */
    _createMonthPickerSlot() {
      var names = qx.locale.Date.getMonthNames(
        "wide",
    ...
    __onPickerChangeSelection(e) {
      if (e.getData().slot > 0) {
        ...
        this._updatePickerTimer = setTimeout(
          function () {
            this._updatePickerDaySlot();
          }.bind(this),
          250
    ...
    _updatePickerDaySlot() {
      ...
      var slotData = this._createDayPickerSlot(
        monthIndex,
     ...
     * @param e
     */
    __onPickerConfirmSelection(e) {
      this.__resultsLabel.setValue("");

      for (var i = 0; i < e.getData().length; i++) {
        var data = e.getData()[i];
        this.__resultsLabel.setValue(
          this.__resultsLabel.getValue() +
            " Received <b>confirmSelection</b> from Picker Dialog. [slot: " +
            data.slot +
            "] [item: " +
            data.item +
            "] <br>"
